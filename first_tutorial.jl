...
# ╔═╡ 602e2f01-9461-43d4-bd16-37b425a4f5a8
using CarboKitten.Production:production_profile

# ╔═╡ 6a2baab0-da2b-406d-9545-29b1f5c6d88f
using GLMakie
...
In this example, the sea level is given as a function. The function should take quantities in units of time and return a quantity in units of length. You can also input a measured sea-level curve (we will introduce it later). 
...
Three benthic biological facies are distinguished based on sediment produced by three carbonate factories:

- T(Tropical) factory (max growth rate = 500.0 m/Myr, tag: euphotic)
- M(Mounds) factory (max growth rate = 400.0 m/Myr, tag: oligophotic)
- C(Cool water) factory (max growth rate = 100.0 m/Myr, tag: aphotic)
...
CarboKitten is also able to input pelagic facies.
"""
...
let
	...
	function plot_production_curve(Facies)
	    ...
	    for (k, prod) in enumerate(Facies)
		    f = production_profile(input, prod.production)
...
begin
	function obtain_sl(path::String)
		input_sl = DataFrame(readdlm(path, '\t', header=false), :auto)
...
own_sealevel_output = run_model(Model{ALCAP}, input_ob, "$(OUTPUTDIR)/tutorial_sl.h5")
...
Note that, due to some unimpletement issue in Interpolation package, we have to first strip the units.
"""

# ╔═╡ 85c4af97-3cbc-4a86-8a15-80997898cc09
begin
	INSO_PATH = "data/input/inso.txt"
	
	function obtain_inso(path)
		input_inso_curve = DataFrame(readdlm(path, '\t', header=false),:auto)
		...
		return linear_interpolation(ustrip.(u"kyr", time_inso), inso_intensity)
	end
...
input_inso = ALCAP.Input(
    ...
    subsidence_rate=50.0u"m/Myr",
    disintegration_rate=50.0u"m/Myr",
    insolation = t -> inso_curve(ustrip(u"kyr", uconvert(u"kyr", t))),
    sediment_buffer_size=50,
    depositional_resolution=0.5u"m",
    facies=FACIES)

# ╔═╡ b727b5c9-538d-4e14-9edb-e2e4cbf749a0
own_inso_output = run_model(Model{ALCAP}, input_inso, "$(OUTPUTDIR)/tutorial_inso.h5")
